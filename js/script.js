...
lightbox.addEventListener('click', function (event) {
  // Close if clicking outside the image (overlay) OR clicking the image itself if desired?
  // User said "clicking anywhere". But we have controls (prev/next).
  // Let's close unless the target is a control button.
  if (!event.target.closest('.lightbox-controls') && !event.target.closest('.lightbox-content .caption-text') && !event.target.closest('.lightbox-content .caption-counter')) {
    toggleLightbox();
  }
});
